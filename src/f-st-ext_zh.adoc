...
浮点最小数和最大数指令 FMIN.S 和 FMAX.S 分别将 _rs1_ 和 _rs2_ 中较小或较大的值 写入 _rd_。仅针对这些指令，值 latexmath:[$-0.0$] 被认为小于值 latexmath:[$+0.0$]。如果两个输入都是 NaN，则结果为规范 NaN（Canonical NaN）。如果只有一个操作数是 NaN，则结果为非 NaN 的那个操作数。如果输入的是信号 NaN（Signaling NaN），即使计算结果不是 NaN，也会触发无效操作异常标志（InvalidOperation Exception Flag）。
...
融合乘加指令在乘数为无穷大（latexmath:[$\infty$]）且被乘数为零时，必须设置无效操作异常标志，即使加数是安静 NaN（quiet NaN）。  

[NOTE]
====
IEEE 754-2008 标准允许，但不强制要求，在执行 latexmath:[$\infty \times 0 +$] 安静 NaN 这一运算时触发无效操作异常。
====
...
[NOTE]
====
FMV.W.X 和 FMV.X.W 指令之前被称为 FMV.S.X 和 FMV.X.S。使用 W 更符合它们作为一种指令的语义，即移动 32 位数据而不对其进行解释。随着 NaN-boxing 定义的清晰，这一点变得更加明确。为了避免影响现有代码，工具将同时支持 W 版和 S 版指令。
====
...
FLT.S 和 FLE.S 执行 IEEE 754-2008 标准所称的 _信号_ 比较（signaling comparison），即如果任一输入为 NaN，则设置无效操作异常标志。FEQ.S 执行 _安静_ 比较（quiet comparison），仅当任一输入为信号 NaN 时才会设置无效操作异常标志。对于这三条指令，如果任一操作数为 NaN，则结果为 0。
