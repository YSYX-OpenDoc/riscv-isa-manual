...
=======
除非另有说明，否则如果浮点操作的结果是 NaN，则它是规范 NaN。规范 NaN 具有正号，所有有效位都清除，除了 MSB，即静默位。对于单精度浮点，这对应于数值 `0x7fc00000`。
>>>>>>> dev_3
(((NaN, generation)))
(((NaN, propagation)))
...
=======
融合乘加指令必须在乘数为 latexmath:[$\infty$] 和零时设置无效操作异常标志，即使加数是静默 NaN。

[NOTE]
====
IEEE 754-2008 标准允许但不要求在操作 latexmath:[$\infty\times 0\ +$]静默 NaN 时引发无效异常。
>>>>>>> dev_3
====
