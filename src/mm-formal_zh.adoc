...
[NOTE]
====
可以观察到， Store_ea 可以在存储的值确定之前发生。例如，为了让操作模型允许试金石测试 LB+fence.r.rw+data-po（如同 RVWMO 一样），Hart 1 中的第一个存储操作必须在其值确定之前执行 Store_ea 步骤，以便第二个存储操作可以看到它是写入到一个不重叠的内存足迹，从而允许第二个存储操作在不违反一致性的情况下乱序提交。
