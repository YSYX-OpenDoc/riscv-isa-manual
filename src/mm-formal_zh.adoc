...
[disc]
* <<do_amo, Satisfy, 提交并传播 AMO 的操作>>：这是满足加载操作、执行所需算术运算，并传播存储操作所需的所有转换的原子执行。
...
对于非 AMO 加载指令或 AMO 指令的指令实例 latexmath:[$i$]，在 AMO 上下文的 <<do_amo, Saitsfy, commit and propagate operations of an AMO>> 转换中，任何 latexmath:[$i.\textit{mem\_loads}$] 中的内存加载操作 latexmath:[$mlo$]，如果其有未满足的片段，且满足 <<sat_by_forwarding，通过从未传播的存储中转发来满足内存加载操作>> 的所有条件，则可以通过从内存中转发未传播的存储来满足。操作：令 latexmath:[$msoss$] 为覆盖 latexmath:[$mlo$] 未满足片段的内存存储操作片段，并应用 <<do_amo，通过从未传播的存储中转发来满足内存操作>> 的操作。
[NOTE]
====
