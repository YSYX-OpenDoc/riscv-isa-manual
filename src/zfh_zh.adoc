...
本章描述了 Zfh 标准扩展，用于支持符合 IEEE 754-2008 算术标准的 16 位半精度二进制浮点指令。Zfh 扩展依赖于单精度浮点扩展 F。<<nanboxing>> 中描述的 NaN-装箱（NaN-boxing）方案得到了扩展，以允许将半精度值使用 NaN-装箱 在单精度值内（当 D 或 Q 扩展可用时，该值还可以递归地作为 NaN-装箱 在双精度或四精度值中）。
