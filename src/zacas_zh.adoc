== "Zacas" 扩展用于原子比较并交换（CAS）指令，版本 1.0.0

=== 介绍

比较并交换（CAS）提供了一种简单且通常更快的方式来执行线程同步操作，当作为硬件指令支持时。CAS 通常用于无锁和无等待算法。本扩展提议 CAS 指令操作 32 位、64 位和 128 位（仅 RV64）数据值。CAS 指令支持 C++11 原子比较和交换操作。
