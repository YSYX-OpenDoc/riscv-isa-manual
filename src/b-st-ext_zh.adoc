...
位操作（bitmanip）扩展集由几个基本 RISC-V 架构的扩展组成，旨在提供代码大小减少、性能提升和功耗减少的组合。虽然这些指令都被设计为具有广泛的适用性，但某些指令在某些领域中的效果比在其他领域更为显著。因此，提供了几个较小的位操作扩展，而不是一个大的扩展。每个较小的扩展根据共同的功能和使用场景进行分组，每个扩展都有其独立的 Zb*-<扩展名称>。
...
[NOTE, caption="Imp" ]
===============================================================
逻辑非指令可以通过对基础所需的 AND、OR 和 XOR 逻辑指令的 _rs2_ 输入进行反转来实现。在某些实现中，用于减法的 rs2 上的反相器可以被用于此目的。
===============================================================
...
描述::
此指令返回在 _rs2_ 指定的索引处清除单比特位的 _rs1_。索引从 _rs2_ 的低 log2(XLEN) 位读取。
...
描述::
此指令返回在 _shamt_ 指定的索引处清除单比特位的 _rs1_。索引从 _shamt_ 的低 log2(XLEN) 位读取。对于 RV32，对应于 shamt[5]=1 的编码是保留的。
...
描述::
此指令计算从最高有效位（即 XLEN-1）开始到第一个 1 之前 0 的数量，并进展到位 0。因此，如果输入为 0 ，输出为 XLEN ；如果输入的最高有效位为 1 ，输出为 0。
...
GCC 内建函数 `+__builtin_popcount (unsigned int x)+` 在 RV32 上由 cpop 实现，在 RV64 上由 *cpopw* 实现。GCC 内建函数 `+__builtin_popcountl (unsigned long x)+` 在 LP64 上由 *cpop* 在 RV64 上实现。
===============================================================
...
描述::
此指令计算从最低有效位（即 0）开始到第一个 1 之前的 0 的数量，并进展到最高有效位（即 XLEN-1）。因此，如果输入为 0，输出为 XLEN；如果输入的最低有效位为 1，输出为 0。
...
描述::
通过按位逻辑 OR 将每个字节内的位进行合并。如果 _rs_ 的对应字节内没有任何位被设置，则将结果寄存器 _rd_ 中该字节的所有位都设置为零；如果 _rs_ 中对应字节内有任何位被设置，则结果寄存器 _rd_ 中该字节的所有位都 1。
