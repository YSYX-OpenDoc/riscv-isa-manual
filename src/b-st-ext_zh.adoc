...
位操作（bitmanip）扩展集由几个基本 RISC-V 架构的扩展组成，旨在提供代码大小减少、性能提升和功耗减少的组合。
虽然这些指令都被设计为具有广泛的适用性，但某些指令在某些领域中的效果比在其他领域更为显著。
...
位操作扩展遵循 RV64 中的约定，带有 _w_ 后缀的指令（_w_ 前没有点）会忽略其输入的高 32 位，操作低 32 位作为有符号值，并生成一个符号扩展到 XLEN 的 32 位有符号结果。
