...
C.ADDI4SPN 是 CIW 格式指令，将零扩展的非零立即数（按 4 缩放）添加到堆栈指针 `x2`，并将结果写入 `rd′`。此指令用于生成指向堆栈分配变量的指针，并扩展为 `addi rd′, x2, nzuimm[9:2]`。当 _nzuimm_≠0 时，C.ADDI4SPN 才有效；_nzuimm_=0 的代码点保留。
