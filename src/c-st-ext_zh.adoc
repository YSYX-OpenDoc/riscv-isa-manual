...
由于原始 RISC ISA 没有留下足够的操作码空间来包含这些未计划的压缩指令，因此它们被开发为完整的新 ISA。这意味着编译器需要为不同的压缩 ISA 编写不同的代码生成器。第一代压缩 RISC ISA 扩展（例如 ARM Thumb 和 MIPS16）仅使用固定的 16 位指令大小，这虽然很好的减少了静态代码的大小，但导致动态指令数的增加，从而导致性能低于原始固定宽度的 32 位指令大小。这导致了第二代压缩 RISC ISA 设计的发展，采用具有混合的 16 位和 32 位指令长度（例如 ARM Thumb2、microMIPS、PowerPC VLE），这样性能与纯 32 位指令相似，但显著地节省代码大小。不幸的是，这些不同代的压缩 ISA 彼此不兼容，也不兼容原始的未压缩 ISA，导致文档、实现和软件工具支持的复杂性显著增加。
