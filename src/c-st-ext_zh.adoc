...
当以下情况时，RVC 使用一种简单的压缩方案，提供常见 32 位 RISC-V 指令的较短 16 位版本：

* 立即数或地址偏移量较小，或
* 其中一个寄存器是零寄存器（`x0`）、ABI 链接寄存器（`x1`）或 ABI 堆栈指针（`x2`），或
* 目标寄存器和第一个源寄存器相同，或
...
[TIP]
====
双精度加载和存储是静态和动态指令的重要组成部分，因此有动机将它们包含在 RV32C 和 RV64C 编码中。

虽然单精度加载和存储对于为当前支持的 ABI 编译的基准测试来说不是静态或动态压缩的重要来源，但对于仅提供硬件单精度浮点单元并且具有仅支持单精度浮点数的 ABI 的微控制器，单精度加载和存储在测量的基准测试中将至少与双精度加载和存储一样频繁。因此，有动机在 RV32C 中提供压缩支持。

短距离子程序调用在微控制器的小型二进制文件中更有可能，因此有动机将它们包含在 RV32C 中。
...
由于原始 RISC ISA 没有留下足够的操作码空间来包含这些未计划的压缩指令，因此它们被开发为完整的新 ISA。这意味着编译器需要为单独的压缩 ISA 提供不同的代码生成器。第一代压缩 RISC ISA 扩展（例如 ARM Thumb 和 MIPS16）仅使用固定的 16 位指令大小，这在静态代码大小上有很好的减少，但导致动态指令计数增加，从而导致性能低于原始固定宽度的 32 位指令大小。这导致了第二代压缩 RISC ISA 设计的发展，具有混合的 16 位和 32 位指令长度（例如 ARM Thumb2、microMIPS、PowerPC VLE），因此性能与纯 32 位指令相似，但具有显著的代码大小节省。不幸的是，这些不同代的压缩 ISA 彼此不兼容，也不兼容原始的未压缩 ISA，导致文档、实现和软件工具支持的复杂性显著增加。
...
C.LWSP 从内存加载一个 32 位值到寄存器 _rd_。它通过将零扩展偏移量（按 4 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `lw rd, offset(x2)`。当 _rd_&#x2260;x0 时，C.LWSP 才有效，_rd_=x0 的代码点保留。

C.LDSP 是 RV64C/RV128C 专用指令，从内存加载一个 64 位值到寄存器 _rd_。它通过将零扩展偏移量（按 8 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `ld rd, offset(x2)`。当 _rd_&#x2260;x0 时，C.LDSP 才有效，_rd_=x0 的代码点保留。

C.LQSP 是 RV128C 专用指令，从内存加载一个 128 位值到寄存器 _rd_。它通过将零扩展偏移量（按 16 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `lq rd, offset(x2)`。当 _rd_&#x2260;x0 时，C.LQSP 才有效，_rd_=x0 的代码点保留。

C.FLWSP 是 RV32FC 专用指令，从内存加载一个单精度浮点值到浮点寄存器 _rd_。它通过将零扩展偏移量（按 4 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `flw rd, offset(x2)`。

C.FLDSP 是 RV32DC/RV64DC 专用指令，从内存加载一个双精度浮点值到浮点寄存器 _rd_。它通过将零扩展偏移量（按 8 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `fld rd, offset(x2)`。
...
C.SWSP 将寄存器 _rs2_ 中的 32 位值存储到内存中。它通过将零扩展偏移量（按 4 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `sw rs2, offset(x2)`。

C.SDSP 是 RV64C/RV128C 专用指令，将寄存器 _rs2_ 中的 64 位值存储到内存中。它通过将零扩展偏移量（按 8 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `sd rs2, offset(x2)`。

C.SQSP 是 RV128C 专用指令，将寄存器 _rs2_ 中的 128 位值存储到内存中。它通过将零扩展偏移量（按 16 缩放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `sq rs2, offset(x2)`。

C.FSWSP 是 RV32FC 专用指令，将浮点寄存器 _rs2_ 中的单精度浮点值存储到内存中。它通过将零扩展偏移量（按 4 缂放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `fsw rs2, offset(x2)`。

C.FSDSP 是 RV32DC/RV64DC 专用指令，将浮点寄存器 _rs2_ 中的双精度浮点值存储到内存中。它通过将零扩展偏移量（按 8 缂放）添加到堆栈指针 `x2` 来计算有效地址。它扩展为 `fsd rs2, offset(x2)`。

[NOTE]
====
函数入口/出口处的寄存器保存/恢复代码占静态代码大小的很大一部分。RVC 中基于堆栈指针的压缩加载和存储在减少保存/恢复静态代码大小方面非常有效，减少了 2 倍，同时通过减少动态指令带宽提高了性能。

其他 ISA 中常用的一种机制是加载多个和存储多个指令，以进一步减少保存/恢复代码大小。我们考虑过将这些指令用于 RISC-V，但注意到以下缺点：
...
虽然合理的架构师可能会得出不同的结论，但我们决定省略加载和存储多个指令，而是使用调用保存/恢复微代码例程的软件方法，以实现最大的代码大小减少。
====
...
C.LW 从内存加载一个 32 位值到寄存器 `_rd′_`。它通过将零扩展偏移量（按 4 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `lw rd′, offset(rs1′)`。

C.LD 是 RV64C/RV128C 专用指令，从内存加载一个 64 位值到寄存器 `_rd′_`。它通过将零扩展偏移量（按 8 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `ld rd′, offset(rs1′)`。

C.LQ 是 RV128C 专用指令，从内存加载一个 128 位值到寄存器 `_rd′_`。它通过将零扩展偏移量（按 16 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `lq rd′, offset(rs1′)`。

C.FLW 是 RV32FC 专用指令，从内存加载一个单精度浮点值到浮点寄存器 `_rd′_`。它通过将零扩展偏移量（按 4 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `flw rd′, offset(rs1′)`。

C.FLD 是 RV32DC/RV64DC 专用指令，从内存加载一个双精度浮点值到浮点寄存器 `_rd′_`。它通过将零扩展偏移量（按 8 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `fld rd′, offset(rs1′)`。
...
C.SW 将寄存器 `_rs2′_` 中的 32 位值存储到内存中。它通过将零扩展偏移量（按 4 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `sw rs2′, offset(rs1′)`。

C.SD 是 RV64C/RV128C 专用指令，将寄存器 `_rs2′_` 中的 64 位值存储到内存中。它通过将零扩展偏移量（按 8 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `sd rs2′, offset(rs1′)`。

C.SQ 是 RV128C 专用指令，将寄存器 `_rs2′_` 中的 128 位值存储到内存中。它通过将零扩展偏移量（按 16 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `sq rs2′, offset(rs1′)`。

C.FSW 是 RV32FC 专用指令，将浮点寄存器 `_rs2′_` 中的单精度浮点值存储到内存中。它通过将零扩展偏移量（按 4 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `fsw rs2′, offset(rs1′)`。

C.FSD 是 RV32DC/RV64DC 专用指令，将浮点寄存器 `_rs2′_` 中的双精度浮点值存储到内存中。它通过将零扩展偏移量（按 8 缂放）添加到寄存器 `_rs1′_` 中的基地址来计算有效地址。它扩展为 `fsd rs2′, offset(rs1′)`。
...
C.ADDI4SPN 是 CIW 格式指令，将零扩展的非零立即数（按 4 缂放）添加到堆栈指针 `x2`，并将结果写入 `rd′`。此指令用于生成指向堆栈分配变量的指针，并扩展为 `addi rd′, x2, nzuimm[9:2]`。当 _nzuimm_≠0 时，C.ADDI4SPN 才有效；_nzuimm_=0 的代码点保留。
...
<<rvcopcodemap>> 显示了 RVC 的主要操作码映射。表的每一行对应于编码空间的一个象限。最后一个象限，具有两个最低有效位设置，对应于宽于 16 位的指令，包括基本 ISA 中的指令。某些指令仅对某些操作数有效；当无效时，它们标记为 _RES_，表示该操作码保留用于未来的标准扩展；_Custom_ 表示该操作码指定用于自定义扩展；或 _HINT_ 表示该操作码保留用于微架构提示（见 <<rvc-hints, 第 18.7 节>>）。
