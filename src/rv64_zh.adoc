...
`ADDIW` 是一个 RV64I 指令，它将符号位扩展的 12 位立即数与寄存器 _rs1_ 的值相加，得到一个32位结果，并将这一 32 位结果的符号位扩展写入到 _rd_ 中。计算出来的 32 位结果的溢出应当被忽略，最终写入的结果应当是符号位扩展到 64 位的 32 位结果。注意，ADDIW _rd, rs1, 0_ 将寄存器 _rs1_ 的低 32 位的符号位扩展写入寄存器 _rd_（汇编伪指令 SEXT.W）。
