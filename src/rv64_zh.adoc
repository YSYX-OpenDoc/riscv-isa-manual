...
大多数整数计算指令在 XLEN 位值上进行操作。
RV64I 提供了额外的指令变体来操作 32 位值，这些指令通过操作码的后缀 'W' 表示。这些 "*W" 指令忽略输入的高 32 位，并始终生成 32 位有符号值，将其符号扩展到 64 位，即 XLEN-1 到 31 位相等。

[NOTE]
====
编译器和调用约定保持一个不变的规则，即所有 32 位值都以符号位扩展格式保存在 64 位寄存器中。
即使是 32 位无符号整数，也会将第 31 位扩展到第 63 到 32 位。
因此，无符号和有符号 32 位整数之间的转换无需任何操作，从有符号 32 位整数到有符号 64 位整数的转换也是如此。在这一不变规则下，现有的 64 位宽 SLTU 和无符号分支比较仍然可以正确操作 32 位无符号整数。同样，现有的 64 位宽逻辑操作对 32 位符号位扩展整数保持符号位扩展属性。为了确保 32 位值的合理性能，需要增加一些新指令（ADD[I]W/SUBW/SxxW）用于加法和移位操作。
====
(((RV64I, shifts)))
(((RV64I, compares)))
...
依靠常量进行的移位操作被特化编码为 I 型格式，使用与 RV32I 相同的指令操作码。要移位的操作数在 _rs1_ 中，移位量编码在 RV64I 的 I 型立即数字段的低 6 位中。右移类型编码在第 30 位。SLLI 是逻辑左移（低位补零）；SRLI 是逻辑右移（高位补零）；SRAI 是算术右移（原始符号位被复制到空出来的高位）。
(((RV64I, SLLI)))
(((RV64I, SRKIW)))
(((RV64I, SRLIW)))
(((RV64I, RV64I-only)))
...
AUIPC（将上半部分立即数加到 `pc`）使用与 RV32I 相同的操作码。AUIPC 用于构建 `pc` 相对地址，并使用 U 型格式。AUIPC 从 U 型立即数形成 32 位偏移量，最低 12 位填充为零，同时符号位扩展到 64 位，然后与 AUIPC 指令的地址相加，将结果放入寄存器 _rd_。
