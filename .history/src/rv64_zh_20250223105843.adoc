...
[NOTE]
====
编译器和调用约定保持一个不变式：所有 32 位值都以符号位扩展格式储存在 64 位寄存器中。因此，即使是 32 位无符号整数，也会将第 31 位扩展到第 63 到 32 位。因此，符号扩展后的 32 位无符号整数与有符号整数之间的转换是一个空操作，从有符号 32 位整数转换到有符号 64 位整数也是如此。在这一不变式下，现有的 64 位宽的 SLTU 和无符号分支比较仍然能正确操作无符号 32 位整数。同样，现有的 64 位宽逻辑操作在处理符号扩展的 32 位整数时，会保持符号位扩展的属性。为了确保 32 位值的合理性能，需要增加一些新指令（ADD[I]W/SUBW/SxxW）用于加法和移位操作。
====
(((RV64I, shifts)))
(((RV64I, compares)))
...
`ADDIW`（Add Immediate Word）是一个 RV64I 指令，它将符号位扩展的 12 位立即数与寄存器 _rs1_ 的值相加，得到一个32位结果，并将这一 32 位结果的符号位扩展写入到 _rd_ 中。计算出来的 32 位结果的溢出应当被忽略，最终写入的结果应当是符号位扩展到 64 位的 32 位结果。注意，ADDIW _rd, rs1, 0_ 将寄存器 _rs1_ 的低 32 位的符号位扩展写入寄存器 _rd_（汇编伪指令 SEXT.W）。
...
`SLLIW`（Shift Left Logical Immediate Word）、`SRLIW`（Shift Right Logical Immediate Word）和 `SRAIW`（Shift Right Arithmetic Immediate Word）是 RV64I 的专用指令，它们的定义与前述指令类似，但对 32 位值进行操作，并将其 32 位结果符号位扩展到 64 位。具有 imm[5] ≠ 0 的 SLLIW、SRLIW 和 SRAIW 编码是保留的。
...
`LUI`（Load Upper Immediate）使用与 RV32I 相同的操作码。LUI 将 32 位 U 型立即数放入寄存器 _rd_，最低 12 位填充为零。然后再将 32 位结果符号位扩展到 64 位。
(((RV64I, LUI)))

`AUIPC`（Add Upper Immediate to PC）使用与 RV32I 相同的操作码。AUIPC 用于构建 `pc` 相对地址，并使用 U 型指令格式。AUIPC 从 U 型立即数形成 32 位偏移量，最低 12 位填充为零，同时符号位扩展到 64 位，然后与 AUIPC 指令的地址相加，将结果放入寄存器 _rd_。
...
`ADDW`（Add Word）和 `SUBW`（Subtract Word）是 RV64I 专用指令，它们的定义类似于 ADD 和 SUB，但对 32 位值进行操作并生成有符号的 32 位结果。溢出被忽略，结果会被符号位扩展到 64 位并写入目标寄存器。
(((RV64I-only, ADDW)))
(((RV64I-only, SUBW)))

`SLL`、`SRL` 和 `SRA` 对寄存器 _rs1_ 中的值按寄存器 _rs2_ 中的移位量执行逻辑左移、逻辑右移和算术右移操作。在 RV64I 中，仅考虑 _rs2_ 的低 6 位作为移位量。

`SLLW`（Shift Left Logical Word）、`SRLW`（Shift Right Logical Word）和 `SRAW`（Shift Right Arithmetic Word） 是 RV64I 专用指令，定义与上述指令类似，但对 32 位值进行操作，并将其 32 位结果符号位扩展到 64 位。移位量由 _rs2[4:0]_ 给出。
(((RV64I-only, SLLW)))
(((RV64I-only, SRLW)))
(((RV64I-only, SRAW)))
